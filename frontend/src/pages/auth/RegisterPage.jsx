...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const validationMessage = Array.isArray(err?.details?.errors) ? err.details.errors[0]?.msg : null;
      const message =
        validationMessage ||
        err?.details?.error ||
        err?.message ||
      ...
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-7xl items-center justify-center">
        <div className="w-full max-w-md rounded-3xl border border-slate-200/80 bg-white/90 px-6 pb-8 pt-8 shadow-[0_16px_44px_rgba(15,23,42,0.12)] backdrop-blur-sm transition-all duration-300 sm:px-8">
          <div className="text-center">
            ...
            <p className="mt-3 text-base text-slate-500 sm:text-lg">Start your study journey</p>
          </div>

          <form onSubmit={handleSubmit} className="mt-8 space-y-5 sm:space-y-6">
            <div>
              <label className="mb-2 block text-sm font-semibold uppercase tracking-[0.08em] text-slate-700">Username</label>
            ...
            <div>
              <label className="mb-2 block text-sm font-semibold uppercase tracking-[0.08em] text-slate-700">Confirm password</label>
              <div className="relative">
                <div
                  className={`pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 ${
